...
import { ERC721Facet } from "../erc721/facets/ERC721Facet.sol";
import { MintRoleFacet } from "../erc721/facets/MintRoleFacet.sol";
...
contract TestERC721 is DSTestPlus, UsingDiamondDeployer {
	IERC721 token;
	...
	function setUp() public {
		...
		token = IERC721(address(diamond));
	}

	function testInvariantMetadata() public {
		assertEq(token.name(), "TEST_NAME");
		assertEq(token.symbol(), "TEST_SYMBOL");
	}
	...
	function testMetadata() public {
		bytes32 name = "TEST_NAME";
		bytes32 symbol = "TEST_SYMBOL";

		string memory _name = "TEST_NAME";
		string memory _symbol = "TEST_SYMBOL";

		IERC721 tkn = IERC721(address(new ERC721Facet(name, symbol)));

		assertEq(tkn.name(), _name);
		assertEq(tkn.symbol(), _symbol);
	}
	...
	function testSupportsInterface(bytes4 randomInterface) public {
		...
		interfaces[0] = 0x01ffc9a7;
		interfaces[1] = 0x80ac58cd;
		interfaces[2] = 0x5b5e139f;

		for (uint256 i = 0; i < interfaces.length; i++) {
			assertTrue(IERC165(address(token)).supportsInterface(interfaces[i]));
		}

		assertFalse(IERC165(address(token)).supportsInterface(randomInterface));
	}
}
